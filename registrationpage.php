<?php 
	...
	if($_SERVER["REQUEST_METHOD"] == "POST"){ //this form processor runs when user hits submit
		...
		if(empty($usernameerr) && empty($passworderr) && empty($confirmpasserr) && empty($nameerr)){ //if all the errorfields are empty, meaning nothing went wrong...
			
			$sql = "INSERT INTO logins (Username, Password) VALUES (?, ?)"; //then go ahead and insert the new data into the table
			if($stmt = mysqli_prepare($link, $sql)){
				...
				if(mysqli_stmt_execute($stmt)){
					echo "I think it went right inserting"; //afterwards redirect to login page
				}
				else {
			...
			$sql2 = "INSERT INTO `patients table` (`Patient Name`, LoginIDnumber) VALUES (?,?)";
...
<html lang="en">
	...
	<div class="form-group <?php echo (!empty($confirmpasserr)) ? 'has error' : ''; ?>">
		...
		<input type="password" name="confirmpass" class="form-control" value = "<?php echo $confirmpass; ?>"></center>
		<span class="help-block"><?php echo $confirmpasserr;?></span>
	</div>
	<div class="form-group <?php echo (!empty($nameerr)) ? 'has error' : ''; ?>">
		<center><label>Name (First Last):</label>
